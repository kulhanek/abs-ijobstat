// =============================================================================
//  ABS - Advanced Module System - Common Library
// -----------------------------------------------------------------------------
...
CABSStatServer::CABSStatServer(void)
{
    Terminated = false;
}
...
int CABSStatServer::Init(int argc, char* argv[])
{
    ...
    vout << endl;
    vout << "# ==============================================================================" << endl;
    vout << "# ams-isoftstat (ABS utility) started at " << dt.GetSDateAndTime() << endl;
    vout << "# ==============================================================================" << endl;
...
void CABSStatServer::CtrlCSignalHandler(int signal)
{
...
void CABSStatServer::Finalize(void)
{
    CSmallTimeAndDate dt;
    dt.GetActualTimeAndDate();
    ...
    vout << endl;
    vout << "# ==============================================================================" << endl;
    vout << "# ams-isoftstat (ABS utility) terminated at " << dt.GetSDateAndTime() << endl;
    vout << "# ==============================================================================" << endl;
...
bool CABSStatServer::LoadConfig(void)
{
...
bool CABSStatServer::InitServer(void)
{
...
bool CABSStatServer::ExecuteServer(void)
{
...
bool CABSStatServer::WriteDataToDatabase(CJobStatDatagram& datagram)
{
    ...
    sql_exec.GetInputItem(4)->SetInt(GetKeyID(datagram.GetBatchServer()));
    sql_exec.GetInputItem(5)->SetInt(GetKeyID(datagram.GetQueue()));
    sql_exec.GetInputItem(6)->SetInt(datagram.GetNCPUs());
    sql_exec.GetInputItem(7)->SetInt(datagram.GetCPUMem());
    sql_exec.GetInputItem(8)->SetInt(datagram.GetNGPUs());
    sql_exec.GetInputItem(9)->SetInt(datagram.GetGPUMem());
    sql_exec.GetInputItem(10)->SetInt(datagram.GetNumOfNodes());
    sql_exec.GetInputItem(11)->SetInt(datagram.GetWallTime());
    sql_exec.GetInputItem(12)->SetInt(datagram.GetFlags());
    sql_exec.GetInputItem(13)->SetTimeAndDate(datagram.GetTimeAndDate());

    CSmallString sql;

    sql = "INSERT INTO \"STATISTICS\" (\"Site\",\"User\",\"HostName\",\"HostGroup\","
          "\"BatchServer\",\"Queue\",\"NCPUS\",\"CPUMem\",\"NGPUS\",\"GPUMem\",\"NNODES\","
          "\"WallTime\",\"Flags\",\"Time\") "
          "VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
...
int CABSStatServer::GetKeyID(const CSmallString& key)
{
    // find key id
...
bool CABSStatServer::ShutdownServer(void)
{
    Terminated = true;
    close(Socket);
    return(true);
...
bool CABSStatServer::IsClientAuthorized(const char* p_name)
{
...
const CSmallString CABSStatServer::GetDatabaseName(void)
{
...
const CSmallString CABSStatServer::GetDatabaseUser(void)
{
...
const CSmallString CABSStatServer::GetDatabasePassword(void)
{
    CSmallString setup;
    CXMLElement* p_ele = ServerConfig.GetChildElementByPath("config");
    if( p_ele == NULL ) {
...
int CABSStatServer::GetPortNumber(void)
{
    int setup = 32710;
    CXMLElement* p_ele = ServerConfig.GetChildElementByPath("config");
    if( p_ele == NULL ) {
